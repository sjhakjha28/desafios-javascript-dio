...
No momento em que entra no círculo, cada aluno recebe uma pequena ficha que contém um valor de 1 a 500. Depois que o círculo é formado, conta-se, iniciando no aluno que está ao lado da primeira que entrou no círculo, o número correspondente à ficha que o primeiro detém. 
O aluno onde o número contado cair, deve ser retirado do grupo, e a contagem inicia novamente segundo a ficha do aluno que acabou de ser eliminado. Para ficar mais interessante, quando o valor que consta na ficha é par, a contagem é feita no sentido horário e quando o valor que consta na ficha é ímpar, a contagem é feita no sentido anti-horário.
...
while (num != 0) {
    aluno = [];
    for (i = 0; i < num; i++) {
        aluno.push(prompt().split(' '));
    }

    primeiro = parseInt(aluno[0][1]);
    
    if (primeiro % 2 == 0) par(1, -1);
    else impar(1, 1);

    console.log(`Vencedor(a): `, aluno[0][0]);
    num = prompt();
}

function impar(val, cont) {
    for (i = val; i < primeiro + 1; i++) {
        if (cont > aluno.length - 1) cont = 0;
        if (i == primeiro) {
            primeiro = parseInt(aluno[cont][1]);
            aluno.splice(cont, 1);
            if (aluno.length > 1) {
                if (primeiro % 2 == 0) par(1, --cont);
                else impar(1, cont);
            }
            else i = primeiro + 1;
        }
...
function par(val, cont) {
    for (i = val; i < primeiro + 1; i++) {
        if (cont < 0) cont = aluno.length - 1;
        if (i == primeiro) {
            primeiro = parseInt(aluno[cont][1])
            aluno.splice(cont, 1)
            if (aluno.length > 1) {
